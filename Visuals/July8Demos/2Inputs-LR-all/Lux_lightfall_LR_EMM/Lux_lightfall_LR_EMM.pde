/* //<>//
  ...
  Edit to use 2 inputs (left and right)
...
float fallThreshold = 0.01;    // candles go back to the top when their line's amp is lower than this.

int    sceneOneTenorAmpAdjust     = 1000;  // divide
int    sceneTwoTenorAmpAdjust     = 1000;   // multiply
...
//  println("myInput.getAmplitude(6) = " + myInput.getAmplitude(6));
  ...
  if (scene == 1) { //start of piece, measure 1
    // tenor solo:
    myCandle[3].freqPoints(rightInput.getAdjustedFundAsHz(), rightInput.getAmplitude()); //tenor solo controlls this
  ...
  if (scene == 2) { //at first "calida", measure 9
    myCandle[3].highlight(rightInput.getAmplitude() * sceneTwoTenorAmpAdjust, width*0.6, height*0.15); //tenor1 controls this
    ...
    for (int i = 1; i < 9; i++)
    {
      ...
      if(i % 2 == 0) {
        amp   = leftInput.getAmplitude();
        fund  = leftInput.getAdjustedFundAsHz();
      } else {
        amp   = rightInput.getAmplitude();
        fund  = rightInput.getAdjustedFundAsHz();
      } // pick which amplitude and pitch to use
      myCandle[i].circle(amp * 100, fund, width*0.6, height*0.2);
    } // for
  ...
  if (scene == 3) { //at first "pura", measure 17
    myCandle[3].highlight(rightInput.getAmplitude() / sceneThreeTenorAmpAdjust, width*0.45, height*0.3); //tenor1 still controls this
  ...
  if (scene == 5) { //right after the grand pause after "angeli", measure 30
    myCandle[3].freqPoints(rightInput.getAdjustedFundAsHz(), rightInput.getAmplitude() / sceneFiveTenorAmpAdjust); //tenor solo controls this
  ...
  if (scene == 6) { // at "natum" of lower voices (not tenor soli), measure 38
    myCandle[3].highlight(rightInput.getAmplitude() / sceneSixTenorAmpAdjust, width*0.15, height*0.8); //tenor soli control this
    myCandle[1].highlight(leftInput.getAmplitude() / sceneSixTenorAmpAdjust, width*0.15, height*0.8); //tenor soli control this
...
//      if(i == 3)  {  i = i + 1;  }
      ...
      myCandle[i].circle(amp * 100, fund, width*0.2, height*0.6);
    } // for
