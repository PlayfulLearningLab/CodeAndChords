 ...
 *
 */
public class ModuleTemplate_01_02 {
	...
	public ModuleTemplate_01_02(PApplet parent, MicrophoneInput input, String sidebarTitle)
	{
		this.parent	= parent;
		this.input	= input;
		...
		this.curColorStyle	= ModuleTemplate_01_02.CS_RAINBOW;
		...
		//TODO: make initModuleTemplate_01_02() private again, once it can be called from constructor.
		//this.initModuleTemplate_01_02();
	} // ModuleTemplate_01_02
	 ...
	 */
	public void initModuleTemplate_01_02()
	{
	...
	private void addColorStyleButtons(int colorStyleY)
	{
		...
		.setGroup("sidebarGroup")
		.setInternalValue(ModuleTemplate_01_02.CS_RAINBOW);
		...
		.setGroup("sidebarGroup")
		.setInternalValue(ModuleTemplate_01_02.CS_DICHROM);
		...
		.setGroup("sidebarGroup")
		.setInternalValue(ModuleTemplate_01_02.CS_TRICHROM);
		...
		.setGroup("sidebarGroup")
		.setInternalValue(ModuleTemplate_01_02.CS_CUSTOM);
	...
	private void updateColors(float colorStyle)
	{
		...
		// Rainbow:
		if(this.curColorStyle == ModuleTemplate_01_02.CS_RAINBOW)
		{
//			this.rainbow();
...
//			this.applyHSBModulate(colors, hsbColors);
		...
		// Dichromatic:
		if(this.curColorStyle == ModuleTemplate_01_02.CS_DICHROM)
		{
...
//			this.applyHSBModulate(colors, hsbColors);
		...
		// Trichromatic:
		if(this.curColorStyle == ModuleTemplate_01_02.CS_TRICHROM)
		{
...
//			this.applyHSBModulate(colors, hsbColors);
		...
		// Custom:
		if(this.curColorStyle == ModuleTemplate_01_02.CS_CUSTOM)
		{
...
//			this.applyHSBModulate(this.colors, this.hsbColors);
	...
	public String[] getScale(String key, int majMinChrom)
	{
		...
		if(keyPos == -1) {
			throw new IllegalArgumentException("ModuleTemplate_01_02.getScale: key " + key + " is not a valid key.");
		}
		/*
	...
	{*/
		...
		if(keyPos > majorScales.length) {
			throw new IllegalArgumentException("ModuleTemplate_01_02.getScale(int, int): int param " + keyPos + " is greater than majorScales.length (" + majorScales.length + ").");
		}
...
*/
	...
	private void applyColorModulate(float[][] colors, float[][] originalColors)
	{
		if(colors == null || originalColors == null) {
			throw new IllegalArgumentException("ModuleTemplate_01_02.applyColorModulate: one of the float[] parameters is null (colors = " + colors + "; originalColors = " + originalColors);
		} // error checking
	...
	{
		//System.out.println("ModuleTemplate_01_02: theControlEvent.getController() = " + controlEvent.getController());
		...
		if(id % 2 == 1 && id < sliderCutoff && id > 0)
		{
			...
			try	{
				...
				//System.out.println("ModuleTemplate_01_02.controlEvent: string value " + curTextfield.getStringValue() + 
	...
	/*			
		...
		{
			System.out.println("Here for all buttons, yes?");
			// get current color:
			ColorWheel	curCW	= (ColorWheel)controlEvent.getController();
			
			System.out.println("curCW.getValue() = " + curCW.getValue());
			...
			System.out.println("id = " + id + "; color = " + "rgb(" + color.getRed() + ", " + color.getGreen() + ", " + color.getBlue() + ")");
			...
			else
			{
				...
				if(notePos < 0 || notePos > this.colors.length)	{
					throw new IllegalArgumentException("ModuleTemplate_01_02.controlEvent - custom color Textfields: " +
...
*/
	...
	private void fillOriginalColors()
	{
		if(this.colors == null)
		{
			throw new IllegalArgumentException("ModuleTemplate_01_02.fillOriginalColors: this.colors is null.");
		}
	...
	private	void fillHSBColors()
	{
		if(this.colors == null)
		{
			throw new IllegalArgumentException("ModuleTemplate_01_02.fillHSBColors: this.colors is null.");
		}
...
//		notePos	= ((id + 2) / 3) - 9;
		...
		if(id > 63)
		{
			// Canvas:
			if(id == 64)	{
				throw new IllegalArgumentException("ModuleTemplate_01_02.calculateNotePos(int): id 64 should not be passed to this function, as it does not correspond to a note.");
			}
			...
			if(id == 69)
			{
				...
				} else if(this.curColorStyle == ModuleTemplate_01_02.CS_TRICHROM)
				{
	...
	public float getAttackReleaseTransition(int arORt)
	{
		...
		{
			throw new IllegalArgumentException("ModuleTemplate_01_02.getAttackReleaseTransition: parameter " + arORt + " is not a valid position in the array this.attackReleaseTransition.");
		} // error checking
