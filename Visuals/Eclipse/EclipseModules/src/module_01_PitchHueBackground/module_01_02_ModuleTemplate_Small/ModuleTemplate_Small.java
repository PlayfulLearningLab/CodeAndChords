package module_01_PitchHueBackground.module_01_02_ModuleTemplate_Small;
...
//	private boolean menuVis = false;
	...
	String  inputFile  = "Piano Scale Reference Inputs/Chromatic/A Chromatic.wav";
	...
	public ModuleTemplate_Small(PApplet parent, Input input, String sidebarTitle)
	{
		this.parent	= parent;
		this.input	= input;
		...
		this.curColorStyle	= ModuleTemplate_Small.CS_RAINBOW;
...
>>>>>>> wilder*/
	...
	private void addColorStyleButtons(int colorStyleY)
	{
		...
		.setGroup("sidebarGroup")
		.setInternalValue(ModuleTemplate_Small.CS_RAINBOW);
		...
		.setGroup("sidebarGroup")
		.setInternalValue(ModuleTemplate_Small.CS_DICHROM);
		...
		.setGroup("sidebarGroup")
		.setInternalValue(ModuleTemplate_Small.CS_TRICHROM);
		...
		.setGroup("sidebarGroup")
		.setInternalValue(ModuleTemplate_Small.CS_CUSTOM);
	...
	private void updateColors(float colorStyle)
	{
		...
		// Rainbow:
		if(this.curColorStyle == ModuleTemplate_Small.CS_RAINBOW)
		{
			this.rainbow();
		}

		// Dichromatic:
		if(this.curColorStyle == ModuleTemplate_Small.CS_DICHROM)
		{
			this.dichromatic_OneRGB(/*this.rootColor*/this.colors[0]);
		}

		// Trichromatic:
		if(this.curColorStyle == ModuleTemplate_Small.CS_TRICHROM)
		{
			this.trichromatic_OneRGB(/*this.rootColor*/this.colors[0]);
		}

		// Custom:
		if(this.curColorStyle == ModuleTemplate_Small.CS_CUSTOM)
		{			
...
//		String[]	notes	= this.getScale(this.curKeyOffset, this.majMinChrom);
	...
	public String[] getScale(String key, int majMinChrom)
	{
		...
		if(keyPos == -1) {
			throw new IllegalArgumentException("ModuleTemplate_Small.getScale: key " + key + " is not a valid key.");
		}
		/*
	...
	{*/
		...
		if(keyPos > majorScales.length) {
			throw new IllegalArgumentException("ModuleTemplate_Small.getScale(int, int): int param " + keyPos + " is greater than majorScales.length (" + majorScales.length + ").");
		}
...
*/
	...
	private void applyColorModulate(float[][] colors, float[][] originalColors)
	{
		if(colors == null || originalColors == null) {
			throw new IllegalArgumentException("ModuleTemplate_Small.applyColorModulate: one of the float[] parameters is null (colors = " + colors + "; originalColors = " + originalColors);
		} // error checking
	...
	{
		System.out.println("ModuleTemplate_Small: theControlEvent.getController() = " + controlEvent.getController());
		...
		if(id % 2 == 1 && id < sliderCutoff && id > 0)
		{
			...
			try	{
				...
				System.out.println("ModuleTemplate_Small.controlEvent: string value " + curTextfield.getStringValue() + 
...
//			this.inputFile	= "Piano Scale Reference Inputs/" + filename;
		...
		if(controlEvent.getId() > 23 && (controlEvent.getId() % 3 == 2))
		{
			if(this.curColorStyle == ModuleTemplate_Small.CS_CUSTOM)
			{
				...
				if(notePos < 0 || notePos > this.colors.length)	{
					throw new IllegalArgumentException("ModuleTemplate_Small.controlEvent - custom color Textfields: " +
	...
	public float getAttackReleaseTransition(int arORt)
	{
		...
		{
			throw new IllegalArgumentException("ModuleTemplate_Small.getAttackReleaseTransition: parameter " + arORt + " is not a valid position in the array this.attackReleaseTransition.");
		} // error checking
