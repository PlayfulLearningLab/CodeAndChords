...
class Input
{
  ...
  float getAverageFund(int firstInput, int lastInput)
  {
    ...
    if (!(lastInput > firstInput)) {  
      IllegalArgumentException iae = new IllegalArgumentException("InputClassJack.getAverageFund():  lastInput param " + lastInput + " is not greater than firstInput param " + firstInput);
      iae.printStackTrace();
    } // error checking
