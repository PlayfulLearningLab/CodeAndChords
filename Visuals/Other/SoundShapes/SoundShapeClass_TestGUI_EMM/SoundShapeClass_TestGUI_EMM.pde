...
class SoundShape
{
  ...
  /*
  IFLookAndFeel        = defaultLAF;
  IFLookAndFeel        = darkGrayClicked;
*/
  int  xButtonY        = height - 100;  // because x and y selections are at different vertical levels
  int  yButtonY        = height - 80;
  int  pitchButtonX    = 100;  // because pitch and amp selection are at different horizontal levels
  int  ampButtonX      = 80;

  IFButton[]  buttons  = new IFButton[] {
  
      new IFButton("xPitch", pitchButtonX, xButtonY), 
      new IFButton("xAmp", ampButtonX, xButtonY), 
      new IFButton("yPitch", pitchButtonX, yButtonY), 
      new IFButton("yAmp", ampButtonX, yButtonY)
    };
  ...
  /*
   ...
   Or I could just go in and set xLow and xHigh themselves.
...
/*
    this.darkGrayClicked = new IFLookAndFeel(this, IFLookAndFeel.DEFAULT);
    this.darkGrayClicked.baseColor = color(240, saturationMax, 30);
  ...
  void positionShape()
  {
    ...
    if (this.controlsX == 0) {  
      x = input.getAdjustedFundAsHz(1);
    } else {  
      x = input.getAmplitude(1);
    }

    if (this.controlsY == 0) {  
      y = input.getAdjustedFundAsHz(1);
    } else {  
      y = input.getAmplitude(1);
    }
