class RosetteV3Rotate extends RosetteV3Colors
{
  /*
   ...
   Class that shows rosettes based on the amplitude of one input
   ...
   */
   ...
   int  changeInRotation  = 1500;  // higher number = slower rotation
                                  // (amplitude is divided by this number, and the resulting decimal value is added to rotateBy).
...
//    this.tenorCutoff  = tenorCutoff;
  ...
  void run()
  {
    if (input.getAmplitude() > amp) {
      super.rotateBy = (super.rotateBy + (super.amplitude / changeInRotation)) % 360;
    }
