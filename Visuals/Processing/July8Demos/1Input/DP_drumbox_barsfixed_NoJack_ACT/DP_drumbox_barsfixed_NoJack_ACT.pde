/**
 *  08/25
 *  1-input version; runs w/out Jack.
 *
...
int  whichInputMovesBalls  = 1;  // this line moves the balls in the background -- and it's the only one in this version. :)
...
void setup()
{
  ...
  // Since we don't specify the number of inputs, 
  //the sketch will use default audio, not Jack.
...
*/
  ...
  float  redVol  = myIns.getAmplitude() / volAdjust;
  float  orangeVol  = myIns.getAmplitude() / volAdjust;
  float  yellowVol  = myIns.getAmplitude() / volAdjust;
  float  lightGreenVol  = myIns.getAmplitude() / volAdjust;
  float  forestGreenVol  = myIns.getAmplitude() / volAdjust;
  float  periwinkleVol  = myIns.getAmplitude() / volAdjust;
  float  purpleVol  = myIns.getAmplitude() / volAdjust;
  float  fuchsiaVol  = myIns.getAmplitude() / volAdjust;
  float  pinkVol  = myIns.getAmplitude() / volAdjust;
//  float  pinkVol  = fuchsiaVol;

  float  redPitch  = myIns.getAdjustedFund();
  float  orangePitch  = myIns.getAdjustedFund();
  float  yellowPitch  = myIns.getAdjustedFund();
  float  lightGreenPitch  = myIns.getAdjustedFund();
  float  forestGreenPitch  = myIns.getAdjustedFund();
  float  periwinklePitch  = myIns.getAdjustedFund();
  float  purplePitch  = myIns.getAdjustedFund();
  float  fuchsiaPitch  = myIns.getAdjustedFund();
  float  pinkPitch  = myIns.getAdjustedFund();
  ...
  for (j=0; j<volForestGreen; j++)
  {
    ...
    // the following line had a "divide by 0" error; why? pitchForestGreen must have been -1....
