...
class Input
{
  /*
  11/12/2016
  Every time process is called, I get a NullPointerException.
   - Should look at all the variable in process. -- No, just keep moving my print statement.
  ...
  Input(int numInputs, AudioContext audioContext)
  {
    ...
    for (int i = 0; i < this.psArray.length; i++)
    {
      ...
      println("Input.constructor(int, AudioContext): this.psArray[" + i + "] = " + this.psArray[i]);
...
class FrequencyEMM extends FeatureExtractor<Float, float[]> {
   ...
   *
   * (FrequencyEMM is a listener on PowerSpectrum.)
   */
