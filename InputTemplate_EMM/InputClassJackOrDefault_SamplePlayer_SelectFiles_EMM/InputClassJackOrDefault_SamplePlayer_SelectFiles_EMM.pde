...
class Input
{
  ...
  /**
   *  When selectFile is true, pulls up a file selection dialog and sends those to the String[] constructor.
   *  Otherwise, just makes an Input object from the default line in.
   *
   *  @param  selectFile  boolean that determines whether this object will take input from an audio file or the defaul audio in.
   */
  Input(boolean selectFile)
  {
    selectInput("Select a file to use as audio input:", "initInput");
  ...
  void fileSelected(File selection) {
    if (selection == null) {
      ...
      input.initInput(new String[] { selection.getAbsolutePath() });
    } // else
  } // fileSelected
  
  void initInput(File selection)
  {
    if (selection == null) {
      println("Window was closed or the user hit cancel.");
    } else {
      ...
      initInput(new String[] { selection.getName() });
    } // else
  } // initInput(File)
  
  /**
   *  Adds the files in the String[] parameter to a SampleManager group, then to a UGen[],
   *  and calls initInput(UGen[]) on them.
   *
   *  @param  stringArray  A String[] containing the names of files to be used for Input.
   */
  void initInput(String[]  filenames)
  {
